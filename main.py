...
class CryptoUtils:
    ...
    @staticmethod
    def derive_session_key(shared_secret: bytes, context: bytes = b"kgc-device-registration") -> bytes:
    ...
    @staticmethod
    def pack_registration_payload(secret_key: np.ndarray, public_key: np.ndarray) -> bytes:
        """Serialize registration payload as [s || t] with fixed int64 encoding."""
        return secret_key.astype(np.int64).tobytes() + public_key.astype(np.int64).tobytes()

    @staticmethod
    def unpack_registration_payload(payload: bytes, m: int, n: int) -> Tuple[np.ndarray, np.ndarray]:
        """Deserialize registration payload [s || t] back into vectors."""
        secret_size = m * 8
        public_size = n * 8
        if len(payload) != (secret_size + public_size):
            raise ValueError("Invalid registration payload length")

        secret_key = np.frombuffer(payload[:secret_size], dtype=np.int64).reshape((m, 1))
        public_key = np.frombuffer(payload[secret_size:secret_size + public_size], dtype=np.int64).reshape((n, 1))
        return secret_key, public_key


@dataclass
class SecureRegistrationPacket:
    """Encrypted registration payload sent from KGC to IoT device."""
...
class KGC:
    ...
    def register_device_secure(self, device_id: str, device_encapsulation_key: bytes) -> SecureRegistrationPacket:
        """Register a device and securely deliver (s, t) using ML-KEM + AES-GCM."""
        ...
        shared_secret, kem_ciphertext = self.kem.encaps(device_encapsulation_key)
        session_key = CryptoUtils.derive_session_key(shared_secret)
        nonce = os.urandom(12)
        payload = CryptoUtils.pack_registration_payload(s, t)
        aad = device_id.encode("utf-8")
        encrypted_payload = AESGCM(session_key).encrypt(nonce, payload, aad)

        key_gen_time = time.time() - start_time
        METRICS.add_key_gen_time(key_gen_time)
        METRICS.storage_per_device = s.nbytes + t.nbytes + 64

        print(f"[KGC] Device '{device_id}' registered via ML-KEM. Key gen time: {key_gen_time*1000:.3f}ms")
        return SecureRegistrationPacket(
...
class ZKProof:
    ...
    def to_bytes(self) -> int:
        ...
        Only counts what is transmitted over the network:
        - message: variable length
        - challenge: 4 bytes (int32)
        - response (z): M * 8 bytes (main cryptographic component)
        - timestamp: 8 bytes
        - device_id reference: 32 bytes (for public key lookup)
        
        Note: commitment (w) and public_key (t) are NOT transmitted.
        - commitment is recomputed by verifier as w' = A·z - c·t mod q
        - public_key is already registered with verifier via KGC
        """
        return (
            len(self.message.encode()) +  # message
            4 +                            # challenge (c)
            self.response.nbytes +         # response (z) - main component
            8 +                            # timestamp
            32                             # device_id reference for key lookup
        )
...
class IoTDevice:
    ...
    def __init__(self, device_id: str, kgc: KGC, params: SecurityParams = PARAMS):
        ...
        # Device-side ML-KEM keypair for secure registration with KGC.
        self.kem_encapsulation_key, self.kem_decapsulation_key = ML_KEM_768.keygen()

        packet = kgc.register_device_secure(device_id, self.kem_encapsulation_key)
        shared_secret = ML_KEM_768.decaps(self.kem_decapsulation_key, packet.kem_ciphertext)
        session_key = CryptoUtils.derive_session_key(shared_secret)
        aad = device_id.encode("utf-8")
        payload = AESGCM(session_key).decrypt(packet.nonce, packet.encrypted_payload, aad)
        self.secret_key, self.public_key = CryptoUtils.unpack_registration_payload(
            payload,
            self.params.M,
            self.params.N,
        )
...
class BlockchainNode:
    ...
    def verify_single(self, proof: ZKProof) -> bool:
        """Verify a single Zero-Knowledge Proof by recomputing w and checking hash."""
        start_time = time.time()
        ...
        # Verify that H(w' || message) produces the same challenge c
        expected_c, _ = CryptoUtils.shake256_challenge(computed_w, proof.message, self.params.TAU, self.params.Q)
        is_valid = (expected_c == proof.challenge)
