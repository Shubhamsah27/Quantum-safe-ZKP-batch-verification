...
def get_security_params(level: int = 2) -> SecurityParams:
    """
    Get security parameters for the specified NIST security level.
    
    Args:
        level: Security level (0=TOY, 1=NIST-1, 2=NIST-2, 3=NIST-3)
        
    Returns:
        SecurityParams: Configured parameters for the specified level
    """
    if level not in SECURITY_PRESETS:
        raise ValueError(f"Invalid security level: {level}. Must be 0, 1, 2, or 3")
